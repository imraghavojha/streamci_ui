...
export function AnalyticsDashboard({ clerkUserId }: AnalyticsDashboardProps) {
    ...
    // keep separate fetch - this endpoint has different data than realtime dashboard
    useEffect(() => {
    ...
    if (loading) return (
        <div className="text-center py-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card><CardContent className="pt-6 flex justify-center items-center h-40"><Loader2 className="w-6 h-6 animate-spin" /></CardContent></Card>
            <Card><CardContent className="pt-6 flex justify-center items-center h-40"><Loader2 className="w-6 h-6 animate-spin" /></CardContent></Card>
        </div>
    )
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Build Activity - FIXED: Don't use builds_last_24h as running_builds */}
            <Card>
                <CardHeader>
                ...
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                            ...
                            <div className="text-sm text-blue-800 dark:text-blue-300">total builds</div>
                        </div>
                        <div className="text-center p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                            <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">
                                {stats.running_builds || 0}
                            </div>
                            <div className="text-sm text-yellow-800 dark:text-yellow-300">currently running</div>
                        </div>
                    </div>
                    ...
                    <div className="pt-2">
                        <div className="flex justify-between text-sm mb-2">
                            ...
                            <span>{stats.builds_analyzed || stats.total_builds || 0} analyzed</span>
                        </div>
                    ...
                    {trends.failure_patterns && Array.isArray(trends.failure_patterns) && trends.failure_patterns.length > 0 ? (
                        <div className="space-y-3">
                            {trends.failure_patterns.map((pattern: any, index: number) => (
                                <div key={index} className="p-3 border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 rounded">
                                    <div className="flex items-center justify-between">
                                        <span className="font-medium text-red-800 dark:text-red-300">{pattern.repository}</span>
                        ...
                        <div className="text-center py-8">
                            ...
                            <p className="text-green-600 dark:text-green-400 font-medium">all repositories healthy</p>
